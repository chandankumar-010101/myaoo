...
import 'package:pangeachat/pages/settings_stories/settings_stories_view.dart';
...
class SettingsStoriesController extends State<SettingsStories> {
  ...
  Future<void> _loadUsers() async {
    final room = _storiesRoom = await Matrix.of(context).client.getStoriesRoom(context, "");
    if (room == null) {
      noStoriesRoom = true;
      return;
    }
    ...
    final contacts = Matrix.of(context).client.contacts.where((contact) => !users.any((u) => u.id == contact.id));
