...
class _AudioPlayerState extends State<AudioPlayerWidget> {
  ...
  void _playAction() async {
    ...
    onAudioPositionChanged ??= audioPlayer.positionStream.listen((state) {
      setState(() {
        statusText = '${state.inMinutes.toString().padLeft(2, '0')}:${(state.inSeconds % 60).toString().padLeft(2, '0')}';
        currentPosition = ((state.inMilliseconds.toDouble() / maxPosition) * AudioPlayerWidget.wavesCount).round();
      });
    });
    onDurationChanged ??=
        audioPlayer.durationStream.listen((max) => max == null ? null : setState(() => maxPosition = max.inMilliseconds.toDouble()));
  ...
  List<int> _getWaveform() {
    final eventWaveForm = widget.event.content.tryGetMap<String, dynamic>('org.matrix.msc1767.audio')?.tryGetList<int>('waveform');
            ...
            child: status == AudioPlayerStatus.downloading
                ...
                : InkWell(
                    ...
                    child: Material(
                      ...
                      borderRadius: BorderRadius.circular(64),
                      child: Icon(
                        audioPlayer.playerState.playing ? Icons.pause_outlined : Icons.play_arrow_outlined,
                        color: widget.color,
                      ),
                    ),
            ...
            child: Row(
              children: [
                for (var i = 0; i < AudioPlayerWidget.wavesCount; i++)
                  Expanded(
                    child: InkWell(
                      onTap: () => audioPlayer.seek(Duration(milliseconds: (maxPosition / AudioPlayerWidget.wavesCount).round() * i)),
                      child: Container(
                        height: 32,
